	  ...
	  //$("#active_players").append("<li class='players' id='player"+event.data+"'>"+event.data+"</li>")
		...
		var generateTable =function (enemy){
			...
			for (i = 0;i < 10;i++){
				...
				for (j = 0; j<10;j++){
					table+="<td class=\"active-rows table_rows\" onclick=\" if (isCross) $(this).html(cross);else $(this).html(zero);$(this).removeClass(act_rows);$(act_rows).removeAttr(onclick_str);$(turn_div).html((!isTurn)? your_turn:opposite_turn);socket.send($(this)[0].id+madestepName); \" id=\""+i+"x"+j+"\" ></td>"
				}
				table+="</tr>";
			}
			table+="</tbody></table>";
		...
		if (msg.split(":")[0]=="ConfirmClient"){
			...
			onclick_function+= " $(\"#field\").html(generateTable(\""+enemy+"\"));$(this).removeAttr(\"onclick\"); $(\"#play_dialog\").html(\"\");socket.send(\":CheckVal\")})"
			$("#play_dialog").html("<div>user "+enemy+" wants to play against you</div><input type='button' value='submit' onclick='"+onclick_function+"'><input type='button' value='cancel' onclick='$(\"#play_dialog\").html(\"\")'>")
		...
		if (msg.split(":")[0]=="Confirmed"){
			...
			$("#field").html(generateTable(enemy))
